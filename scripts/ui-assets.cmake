...
function(local_archive_extract version out_var out_resolved)
    ...
    if(archives)
        # list(SORT) 比的是字符串，不是数值：直接降序会把 b9899 排在 b10236 之前
        # （'9' > '1'），"取最新"于是取到了较旧的那个。先把 build 号零填充到固定
        # 宽度再降序排，字符串序即数值序，候选链才是真正的从新到旧。
        set(ranked "")
        foreach(archive ${archives})
            get_filename_component(fname "${archive}" NAME_WE)
            string(REGEX REPLACE ".*llama-b([0-9]+)-ui.*" "\\1" arch_num "${fname}")
            if("${arch_num}" STREQUAL "${fname}")
                continue()
            endif()
            string(LENGTH "${arch_num}" arch_len)
            math(EXPR arch_pad "12 - ${arch_len}")
            string(REPEAT "0" ${arch_pad} arch_zeros)
            list(APPEND ranked "${arch_zeros}${arch_num}|b${arch_num}")
        endforeach()
        list(SORT ranked ORDER DESCENDING)
        foreach(entry ${ranked})
            string(REPLACE "|" ";" entry_parts "${entry}")
            list(GET entry_parts 1 arch_ver)
