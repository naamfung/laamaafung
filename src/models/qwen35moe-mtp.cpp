...
void llama_model_qwen35moe_mtp::load_arch_hparams(llama_model_loader & ml) {
    ml.get_key(LLM_KV_EXPERT_FEED_FORWARD_LENGTH,        hparams.n_ff_exp_arr[0], false);
...
void llama_model_qwen35moe_mtp::load_arch_tensors(llama_model_loader &) {
    ...
    const int64_t n_ff_exp   = hparams.n_ff_exp(0) ? hparams.n_ff_exp(0) : n_ff / n_expert_used;
