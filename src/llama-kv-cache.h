...
public:
    ...
    // KVMem: remove cells by their *logical* (cache-row) position, falling back to the
    // physical position for cells that have no logical position recorded.
    ...
    // KVMem: raw V storage tensor of a layer (no view/slot plumbing)
