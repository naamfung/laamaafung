...
也可在 server 啟動後，透過 OpenAI 相容 API 的 per-request 欄位覆蓋推理塊採樣：`reasoning_temp`、`reasoning_top_k`、`reasoning_top_p`、`reasoning_min_p`、`reasoning_repeat_last_n`、`reasoning_repeat_penalty`、`reasoning_frequency_penalty`、`reasoning_presence_penalty`、`reasoning_dry_penalty_last_n` 等。
