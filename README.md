...
本分支以 v18 為基準，合併上游 `b10621`（v0.3.0）並完成引擎側的適配：

- 引擎適配上游 API 變更（`811efaef0`），並昇版本號至 `0.3.0-dev`。
- `llama_mmap::register_host()` 新增 `_WIN32` 支持，使 `GGML_CUDA_REGISTER_HOST=1` 在 Windows 下真正生效，改善 MoE H2D 傳輸。
- 工具 UI 更新為上游最新版，保留本地靜態打包與 gzip 壓縮邏輯。
