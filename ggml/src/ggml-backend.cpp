    ...
    for (int split_id = 0; split_id < sched->n_splits; split_id++) {
        ...
        for (int input_id = 0; input_id < split->n_inputs; input_id++) {
            ...
            if (input->flags & GGML_TENSOR_FLAG_INPUT) {
                ...
                // Note: The expert filtering optimization for MoE weights (bitset operations and multiple set_async calls)
                // introduces overhead in CUDA graph replay. We now skip it and use full tensor copy to avoid sync/idx calculation overhead.
