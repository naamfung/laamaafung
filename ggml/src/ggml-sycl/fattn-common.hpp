...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            sycl::float2 kv = make_float2(TURBO_CENTROIDS_3BIT[idx0] * norm, TURBO_CENTROIDS_3BIT[idx1] * norm);

#ifdef GGML_SYCL_F16
            const sycl::half2 qv_h2 = ((const sycl::half2 *) Q_v)[k_KQ_0/nthreads + k_KQ_1];
            ggml_sycl_mad(sum, kv, __half22float2(qv_h2));
#else
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            sycl::float2 kv = make_float2(TURBO_CENTROIDS_2BIT[idx0] * norm, TURBO_CENTROIDS_2BIT[idx1] * norm);
...
#pragma unroll
        for (int k_KQ_1 = 0; k_KQ_1 < cpy_ne; ++k_KQ_1) {
            ...
            sycl::float2 kv = make_float2(TURBO_CENTROIDS_4BIT[idx0] * norm, TURBO_CENTROIDS_4BIT[idx1] * norm);
